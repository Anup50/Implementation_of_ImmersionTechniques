...
public class KoopaShell : Enemy
{
	...
	private Mario mario;
	private EndlessMario endlessMario;
	...
	void Start()
	{
		...
		endlessMario = FindObjectOfType<EndlessMario>();
	...
	{
		isBeingStomped = true;
		float direction = 1f; // default direction
							  // Use Mario or EndlessMario if available
		if (mario != null)
		{
			direction = mario.transform.localScale.x;
		}
		else if (endlessMario != null)
		{
			direction = endlessMario.transform.localScale.x;
		}

		if (!isRolling)
		{
			// start rolling left/right depending on player's direction
			if (direction == 1)
